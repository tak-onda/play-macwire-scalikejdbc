...
case class Dao(b: SelectSQLBuilder[User], conditions: List[Option[SQLSyntax]] = Nil)(implicit s: DBSession) extends Syntax {
  ...
  def tpl(u: SyntaxProvider[User], p: SyntaxProvider[Project])(rs: WrappedResultSet): (User, Project) = (Users(u.resultName)(rs), Projects(p.resultName)(rs))

  def apply(): Iterable[SearchResult] = {
    withSQL {
      b.where(sqls.toAndConditionOpt(conditions: _*))
    }.map(tpl(u, p)).list().apply().groupBy(tpl => tpl._1).map {
